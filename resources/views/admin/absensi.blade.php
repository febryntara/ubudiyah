...
@section('body')
    <form class="mb-2" action="{{ route('absensi.validasi') }}" method="get">
        {{-- @csrf --}}
        ...
        <button class="btn btn-primary ml-2">buat absen</button>
    </form>
            ...
            <tbody>
                @foreach ($absen as $item => $value)
                    <tr class="text-center" valign="top">
                        ...
                        <td class="py-2 border-[1px] border-black border-collapse">
                            <a href="{{ route('absensi.edit', ['absen' => $value]) }}"
                                class="warning-dashboard-btn text-xs">edit</a>
                            <form class="inline" action="{{ route('absensi.delete', ['absen' => $value]) }}"
                                method="post">
                                @csrf
                                @method('delete')
